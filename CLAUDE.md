...
BentoBox Integration Test Harness: automated smoke testing for BentoBox (Minecraft plugin) and 33 addons. Uses Docker to spin up a Paper Minecraft server and validates that BentoBox core and all addons load correctly. Commands are issued over the server **console** (main thread) via the itzg image's `mc-send-to-console`, not RCON — RCON dispatches commands off the main thread, which Paper 26.2+ rejects with "Cannot perform command async!".
...
| `test_addon_enabled` | Each of 33 addons reports `(ENABLED)` in `bbox v` console output |
| `test_worlds_registered` | 11 game worlds appear in `bbox v` console output (acidisland, boxed, bskyblock, caveblock, chunkblock, oneblock, parkour, poseidon, skygrid, stranger, tradewinds) |
| `test_commands_registered` | 12 key commands respond via the console: `bbox`, `bsbadmin`, `acid`, `obadmin`, `cbadmin`, `chadmin`, `twadmin`, `boxadmin`, `sgadmin`, `padmin`, `stranger`, `parkour` |
...
- If `bbox v` itself throws (e.g. a game mode that failed to enable leaves a null world behind — BentoBox#3089), its output stops before the world and addon lists. Core Load then reports the exception once, and `test_addon_enabled` / `test_worlds_registered` fall back to BentoBox's startup log (`Enabling <Addon> (...)` with no `Skipping <Addon>` / dependency / disabled message after it), so only the addon that really failed is reported instead of every check.
